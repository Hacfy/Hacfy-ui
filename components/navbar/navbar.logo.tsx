...
export function Navbarlogo() {
  ...
  <button
    ...
    onClick={() => setServicesOpen(!servicesOpen)} // toggle on click if needed
  >
    Services
    <ChevronDown
  ...
  {servicesOpen && (
    <div
      className="absolute left-1/2 -translate-x-1/2 mt-4 bg-white border rounded-2xl w-[1000px] max-h-[500px] overflow-y-auto p-8 z-50 shadow-2xl transition-all duration-300 ease-out backdrop-blur-sm border-gray-200"
      style={{
        background:
            ...
            {serviceData[selectedCategory]?.services.map((service, idx) => {
              ...
              return (
                <Link
                  ...
                  className="group p-4 rounded-xl border border-gray-200 transition-all duration-300 bg-white hover:bg-gradient-to-br hover:from-red-50 hover:to-white hover:border-red-300 hover:shadow-xl hover:scale-105"
                  onClick={() => setServicesOpen(false)}
                >
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0 p-2 rounded-lg bg-red-100 group-hover:bg-red-200 transition-colors duration-300">
                      <IconComponent className="w-5 h-5 text-red-600 group-hover:text-red-700 transition-colors duration-300" />
                    </div>
